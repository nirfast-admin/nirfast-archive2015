function [fwd_mesh,pj_error] = reconstruct_stnd_newl(fwd_mesh,...
                                                recon_basis,...
                                                frequency,...
                                                data_fn,...
                                                iteration,...
...
for it = 1 : iteration
  ...
  % Calculate jacobian
  [J,data]=jacobian_stnd_newl(fwd_mesh,frequency,recon_mesh);
  ind = find(data.link(:,3)==0);
  data.amplitude(ind,:) = []; 
  data.phase(ind,:) = []; clear ind
