function [fwd_mesh,pj_error] = reconstruct_fl2(fwd_mesh,...
    recon_basis,...
    frequency,...
    data_fn,...
    iteration,...
...
%*************************************************************
% Iterate
[Jpre,datafl,MASS_m] = jacobian_fl2(fwd_mesh,frequency,data_fwd);

for it = 1 : iteration
    
    % Update Jacobian with fluroescence field 
    if it == 1
        [Jwholem,junk] = update_jacobian_fl(fwd_mesh,MASS_m,frequency,data_fwd,Jpre);
        clear junk
    else
        [Jwholem,datafl] = update_jacobian_fl(fwd_mesh,MASS_m,frequency,data_fwd,Jpre);
    end
